...
object TokenManager {
  def apply[F[_]: Async](oauth: OAuth2[F]): F[TokenManager[F]] =
    Async[F].delay { CaffeineCache[AccessToken](CacheConfig()) }.map { cache =>
      (refreshToken: String) => OptionT(cache.get[F](refreshToken)).getOrElseF(
        oauth.refresh(refreshToken).flatMap { case (v, r) =>
