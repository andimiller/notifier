...
object Runner {
  ...
  def apply[F[_]: Timer: Sync: Logger](tokens: List[String], tokenManager: TokenManager[F], notifications: Notifications[F]): Stream[F, Unit] = {
    val tick: FiniteDuration = (notificationTtl / tokens.length) + 1.second
    for {
      _ <- Stream.eval(Logger[F].info(s"Scheduling jobs, got ${tokens.length} tokens across ${notificationTtl} window, using ${tick} tick rate"))
      ...
      (at, notifs) <- Stream.eval(
        tokenManager.getToken(token).flatMap { at =>
          notifications.getNotifications(at).tupleLeft(at)
        }
      )
