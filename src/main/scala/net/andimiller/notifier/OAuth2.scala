...
import org.http4s.{AuthScheme, BasicCredentials, Credentials, MediaType, Method, Request, Uri, UrlForm, headers}
import org.http4s.client.Client
import org.http4s.headers.{Authorization, Host, `Content-Type`}
...
class OAuth2[F[_]: Sync](
  ...
  def refresh(refreshToken: String): F[(Verified, Refreshed)] =
    client.expect[Refreshed](Request[F](
      Method.POST,
      tokenUri,
    ).withEntity(
      UrlForm(
